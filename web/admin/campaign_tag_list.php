            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1>餐飲情報標籤
                            <small>列表</small>
                            <button type="button" class="btn btn-brand float-right" onclick="location.href='campaign_tag_edit.php?edit_id=0'"><span class="fas fa-plus"></span></button>
                        </h1>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card">
                        <form name="query_form" id="query_form">
                            ...
                            <div class="card-footer">
                                <button type="button" class="btn btn-primary float-right" onclick="query_form_sumbit('campaign_tag_list.php')"><span class="fas fa-search"></span></button>
...
<script>
                ...
                dataSrc:function(json){
                    ...
                    if(json.data!=null){
                        for(var i=0;i<json.data.length;i++){
                            ...
                            var color="<span class='badge' style='color:white; background-color:"+row.color+"'>"+row.color+"</span>";

                            var status={};
                            GetStatusColorString(row.status,status);
                            status="<span class='badge bg-"+status.color+"'>"+status.string+"</span>";

                            var operate="<a class='btn btn-sm btn-primary' data-title='編輯' href='campaign_tag_edit.php?edit_id="+row.id+"'><span class='fas fa-pen-square'></span></a>";
                            operate+="<a class='btn btn-sm btn-danger' data-title='刪除' onclick='delete_alert(\"餐飲情報標籤\","+row.id+",\""+row.name+"\",\"<?= WEBAPI_CAMPAIGN_TAG ?>\"); return false;'><span class='fas fa-trash-alt'></span></a>";
    ...
    function GetUrl()
    {
        var url="_admin_list_ajax.php?api=<?= WEBAPI_CAMPAIGN_TAG ?>";
