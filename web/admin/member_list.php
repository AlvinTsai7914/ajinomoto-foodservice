            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1>會員
                            <small>列表</small>
                            <button type="button" class="btn btn-brand float-right" onclick="location.href='member_edit.php?edit_id=0'"><span class="fas fa-plus"></span></button>
                        </h1>
                    </div>
                </div>
            </div>
                        ...
                        <form name="query_form" id="query_form">
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <label for="q_phone" class="col-md-1">電話</label>
                                    <div class="col-md-2">
                                        <input type="text" id="q_phone" name="q_phone" class="form-control" value="<?= $q_phone ?>">
                                    </div>
                                    ...
                                    <div class="col-md-2">
                                        <select id="q_status" name="q_status" class="form-control" style="width: 100%;">
                                            ...
                                            <option  value="1" <?php echo ($q_status == "1") ? "selected='selected'" : ""; ?>>尚未驗證</option>
                                            <option  value="2" <?php echo ($q_status == "2") ? "selected='selected'" : ""; ?>>停用</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            <div class="card-footer">
                                <button type="button" class="btn btn-primary float-right" onclick="query_form_sumbit('member_list.php')"><span class="fas fa-search"></span></button>
...
<script>
                ...
                dataSrc:function(json){
                    ...
                    if(json.data!==null){
                        for(var i=0;i<json.data.length;i++){
                            ...
                            var gender=GetMemberGenderString(row.gender);

                            var status={};
                            GetMemberStatusColorString(row.status,status);
                            status="<span class='badge bg-"+status.color+"'>"+status.string+"</span>";
                            var operate="";
                            operate+="<a class='btn btn-sm btn-primary' data-title='編輯' href='member_edit.php?edit_id="+row.id+"'><span class='fas fa-edit'></span></a>";
                            operate+="<a class='btn btn-sm btn-danger' data-title='刪除' onclick='delete_alert(\"會員\","+row.id+",\""+row.name+"\",\"<?= WEBAPI_MEMBER ?>\"); return false;'><span class='fas fa-trash-alt'></span></a>";
                            if(row.status==="1")
                                operate+="<a class='btn btn-sm btn-info' data-title='發送帳號驗證信' onclick='send_verification_alert("+row.id+",\""+row.account+"\",\"<?= WEBAPI_MEMBER ?>\"); return false;'><span class='fas fa-envelope-square'></span></a>";
                            operate+="<a class='btn btn-sm btn-warning' data-title='發送重設密碼信' onclick='send_setpassword_alert("+row.id+",\""+row.account+"\",\"<?= WEBAPI_MEMBER ?>\"); return false;'><span class='fas fa-key'></span></a>";
    ...
    function GetUrl()
    {
        var url="_admin_list_ajax.php?api=<?= WEBAPI_MEMBER ?>";
        ...
        var q_phone=$("#q_phone").val().trim();
        if(q_phone)
            query_str+=" AND phone like '%"+q_phone+"%'";
