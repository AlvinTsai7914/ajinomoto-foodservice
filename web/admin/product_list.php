...
$q_brand_id = request_str("q_brand_id");
            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1>商品
                            <small>列表</small>
                            <button type="button" class="btn btn-brand float-right" onclick="location.href='product_edit.php?edit_id=0'"><span class="fas fa-plus"></span></button>
                        </h1>
                    </div>
                </div>
            </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <form name="query_form" id="query_form">
                            <div class="card-body">
                                <div class="row">
                                    <label for="q_brand_id" class="col-md-1">品牌</label>
                                    <div class="col-md-2">
                                        <select id="q_brand_id" name="q_brand_id" class="form-control" style="width: 100%;">
                                            <option  value="">(All)</option>
                                            <?php foreach ($brands as $c) { ?>
                                                <option  value="<?= $c["id"] ?>" <?php echo ($c["id"] == $q_brand_id) ? "selected='selected'" : ""; ?>><?= $c["name"] ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
                            ...
                            <div class="card-footer">
                                <button type="button" class="btn btn-primary float-right" onclick="query_form_sumbit('product_list.php')"><span class="fas fa-search"></span></button>
...
<script>
                ...
                dataSrc:function(json){
                    ...
                    if(json.data!=null){
                        for(var i=0;i<json.data.length;i++){
                            ...
                            var pic_url="";
                            if(row.pic_urls!="")
                                pic_url=(row.pic_urls.split(","))[0];

                            var pic_alt="";
                            if(row.pic_alts!="")
                                pic_alt=(row.pic_alts.split(","))[0];
                            ...
                            var operate="<a class='btn btn-sm btn-primary' data-title='編輯' href='product_edit.php?edit_id="+row.id+"'><span class='fas fa-pen-square'></span></a>";
    ...
    function GetUrl()
    {
        ...
        var query_str="";
        var q_brand_id=$("#q_brand_id").val();
        if(q_brand_id)
            query_str+=" AND brand_id='"+q_brand_id+"'";
