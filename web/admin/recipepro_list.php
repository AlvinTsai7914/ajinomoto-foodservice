...
$q_member_only = request_str("q_member_only");
            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1>專業食譜
                            <small>列表</small>
                            <button type="button" class="btn btn-brand float-right" onclick="location.href='recipepro_edit.php?edit_id=0'"><span class="fas fa-plus"></span></button>
                        </h1>
                    </div>
                </div>
            </div>
                        ...
                        <form name="query_form" id="query_form">
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <label for="q_member_only" class="col-md-1">會員限定</label>
                                    <div class="col-md-2">
                                        <select id="q_member_only" name="q_member_only" class="form-control" style="width: 100%;">
                                            <option  value="" <?php echo ($q_member_only == "") ? "selected='selected'" : ""; ?>>(All)</option>
                                            <option  value="0" <?php echo ($q_member_only == "0") ? "selected='selected'" : ""; ?>>否</option>
                                            <option  value="1" <?php echo ($q_member_only == "1") ? "selected='selected'" : ""; ?>>是</option>
                                        </select>
                                    </div>
                            ...
                            <div class="card-footer">
                                <button type="button" class="btn btn-primary float-right" onclick="query_form_sumbit('recipepro_list.php')"><span class="fas fa-search"></span></button>
                            ...
                            <table style="width: 100%" class="table table-striped table-bordered table-hover" id="table">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width: 10%">會員限定</th>
                                        <th style="width: 10%">瀏覽人數</th>
...
<script>
                ...
                dataSrc:function(json){
                    ...
                    if(json.data!=null){
                        for(var i=0;i<json.data.length;i++){
                            ...
                            var membery_only={};
                            GetMemberOnlyColorString(row.member_only,membery_only);
                            membery_only="<span class='badge bg-"+membery_only.color+"'>"+membery_only.string+"</span>";

                            var status={};
                            GetStatusColorString(row.status,status);
                            status="<span class='badge bg-"+status.color+"'>"+status.string+"</span>";
                            var operate="<a class='btn btn-sm btn-primary' data-title='編輯' href='recipepro_edit.php?edit_id="+row.id+"'><span class='fas fa-pen-square'></span></a>";
                            operate+="<a class='btn btn-sm btn-danger' data-title='刪除' onclick='delete_alert(\"專業食譜\","+row.id+",\""+row.name+"\",\"<?= WEBAPI_RECIPEPRO ?>\"); return false;'><span class='fas fa-trash-alt'></span></a>";
                            return_data.push({
                                'id':row.id,
                                'name':row.name,
                                'pic_url':pic_url,
                                'membery_only':membery_only,
                                'visitor':row.visitor,
    ...
    function GetUrl()
    {
        var url="_admin_list_ajax.php?api=<?= WEBAPI_RECIPEPRO ?>";
        var query_str="";
        var q_name=$("#q_name").val().trim();
        if(q_name)
            query_str+=" AND name like '%"+q_name+"%'";

        var q_member_only=$("#q_member_only").val();
        if(q_member_only)
            query_str+=" AND member_only='"+q_member_only+"'";
