            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-12">
                        <h1>角色
                            <small>列表</small>
                            <button type="button" class="btn btn-brand float-right" onclick="location.href = 'role_edit.php?edit_id=0'"><span class="fas fa-plus"></span></button>
                        </h1>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card">
                        <form name="query_form" id="query_form">
                            ...
                            <div class="card-footer">
                                <button type="button" class="btn btn-primary float-right" onclick="query_form_sumbit('role_list.php')"><span class="fas fa-search"></span></button>
                ...
                dataSrc: function (json) {
                    ...
                    if (json.data != null) {
                        for (var i = 0; i < json.data.length; i++) {
                            ...
                            if (row.id > 1)
                            {
                                operate = "<a class='btn btn-sm btn-primary' data-title='編輯' href='role_edit.php?edit_id=" + row.id + "'><span class='fas fa-pen-square'></span></a>";
                                operate += "<a class='btn btn-sm btn-danger' data-title='刪除' onclick='delete_alert(\"角色\"," + row.id + ",\"" + row.name + "\",\"<?= WEBAPI_ROLE ?>\"); return false;'><span class='fas fa-trash-alt'></span></a>";
                            }
                            return_data.push({
                                'id': row.id,
                                'name': row.name,
